...
async function createImageNodes(urls, node, options) {
  ...
  const fileNodes = (await Promise.all(urls.map(async (url, index) => {
    ...
    try {
      fileNode = await createRemoteFileNode({ ...restOfOptions,
        url,
  ...
  if (fileNodes.length) {
    ...
    await options.cache.set(cacheKey, { ...existingFileNodeMap,
      [name]: fileNodes.map(({
        id
      }) => id)
    });
...
async function getAllFilesUrls(path, node, options) {
  ...
  (await Promise.all(nextValue.map(item => getAllFilesUrls(imagePathSegments[pathIndex + 1], item, options)))).reduce((arr, row) => arr.concat(row), []) : // otherwise, handle leaf node
  nextValue;
}
